...
namespace AdventOfCode
{
    public static class PartTwo
    {
        public static void Run(string inputFile)
        {
            ...
            List<int> emptyCols = Enumerable
                .Range(0, lines[0].Count)
            ...
            List<(int, int)> galaxies = lines
                .SelectMany(
                    (line, i) =>
                        line.Select((thisChar, j) => (thisChar, i, j))
                            .Where(t => t.thisChar == '#')
                            .Select(t => (t.i, t.j))
                )
                .ToList();
                    ...
                    (startGal, i) =>
                        galaxies
                            ...
                            .Select(endGal =>
                            {
                                ...
                                int rowCrosses = emptyRows.Count(cur =>
                                    cur > minRow && cur < maxRow
                                );
                                int colCrosses = emptyCols.Count(cur =>
                                    cur > minCol && cur < maxCol
                                );
